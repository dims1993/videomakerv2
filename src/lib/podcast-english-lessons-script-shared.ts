...
export const PODCAST_MAX_SARA_GOLD_STANDARD = {
  ...
  requiredEditorialBeats: [
    ...
    "flexible catch-up when useful",
    "uniqueMechanism exploration",
    "personal stories from both hosts",
    "simple metaphor or model",
    "counterpoint / nuance",
    "useful phrases / chunks in context",
    "Word Tour payoff",
    "emotional recap",
    "comment question",
    "goodbye",
...
export function buildMaxSaraGoldStandardPromptBlock() {
  ...
  return [
    ...
    `- Micro architecture: animated intro → tension → story → metaphor/model → explanation → language in context → counterpoint → second story → deeper meaning → practical takeaway → Word Tour → emotional recap`,
    ...
    `- Vocabulary/chunks: organic Word Tour payoff — never a dictionary list or “today we will study vocabulary”`,
...
export function buildPodcastEpisodeSkeletonBlock(
  ...
  if (format === "max_sara_conversation") {
    return [
      ...
      "- energetic animated intro (not a dry topic announcement)",
      "- 3,500–4,400 spoken words",
      "- 8–11 parts",
      "- comprehensible input with careful stretch language",
      "- phrases appear in conversation before Word Tour and recur",
      "- personal mini-stories from both hosts",
      "- mild disagreement / nuance at least once",
      "- flexible catch-up (~4–12 turns) that seeds the topic",
      "- one Word Tour payoff (not a classroom list)",
